...
**A routing correction is remembered (2026-08-24).** Re-minting the id stops the distiller staging a twin of the CORRECTED candidate; it does nothing about the mis-route, because the distiller's guess has not changed. `staging._record_route` appends to `routed.jsonl` the destination a human moved knowledge OFF, keyed on the same two hashes the declined ledger uses so a retag is not read as a new decision, and `distill ingest` skips a proposal for a target the sentence was routed away from (`skipped-routed`). Deliberately NOT the declined ledger: a route is not a rejection of the knowledge, and reusing it would suppress the sentence everywhere and block routing back later. Routing back clears the record — the ledger must not outlive the decision it describes.

**Nothing in `core/` took a lock (2026-08-24).** `paths.locked(home, name)` is `fcntl.flock`, held by the read-then-write paths — `staging.route`, `staging.decline`, `flags.consume_flags`, `indexing.rebuild` — and by no read path, since `search` is read-only and a write-shaped wait on the agent's hot path is worse than the race. Advisory and process-scoped on purpose: the OS drops it when the holder dies, so a killed process cannot wedge every later run the way a stale lock FILE does.
