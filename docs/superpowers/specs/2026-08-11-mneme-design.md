...
**Facts location (revised 2026-08-12; write rule superseded 2026-08-13 by user direction).** Facts live *inside* the router skill at `skills/knowledge-index/facts/`; the generated index and the files it routes to are one self-contained directory that travels together. Two resolution rules, both in `units`, and no consumer hardcodes a path:

- **Writes are unconditional.** `facts_write_dir` is always `skills/knowledge-index/facts/`, whatever exists on disk, so a repo scaffolded before this location existed starts filing new topics canonically the moment it takes its next contribution. The 2026-08-12 rule — ask `facts_dir`, which falls back to a top-level `facts/` when the canonical directory is absent — was written as "never fork a repo's layout by an append", and it was the mechanism that made a legacy layout permanent: mneme's own development-knowledge plugin kept receiving new facts at its root forever, because every new fact re-confirmed the layout it was accommodating. The narrow case the old rule was actually protecting survives on its own terms: `harvest._fact_path` appends to a topic file that already exists in *either* layout, since one topic split across two files would give one unit id two homes. Only a topic file that does not exist yet is created, and it is created canonically.
- **Reads tolerate both layouts, until the migration.** Lint, verify, index build, index regeneration, the classify bundle and search all sweep `fact_files`, which covers both directories (`facts_dir`/`facts_dirs`/`find_fact_file` likewise resolve either). A repo that carries both — a legacy tree plus the canonical directory its next write seeds — must never hold knowledge that is committed and on disk yet invisible to retrieval.
