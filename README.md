...
| 15 — Automatic capture | Capture stops depending on the model remembering. A session that did real work and captured nothing says so; a subagent's returned report is mined for findings; a hard-won fix after repeated failures becomes a specific prompt at the next turn (`ORA-06550 then a success of `docker exec`, after 4 failures`), not a generic reminder; and `mneme_flag` over MCP takes text with quotes, `$` and newlines without any shell quoting | ✅ merged (v0.10.0) |
| — | Verified against Claude Code 2.1.278 rather than its docs: `PostToolUse` does carry tool results and `SubagentStop` does carry the subagent's report, both contrary to the documentation, while `Stop` cannot inject context at all. Seeing and speaking are different events, so every trigger records where the knowledge appears and delivers at `UserPromptSubmit` | ✅ merged (v0.10.0) |
| — | Fixes: v0.10.0's `UserPromptSubmit` hook shipped without its executable bit, so it failed with `Permission denied` on every prompt and delivered none of the capture prompts the release was built for. A release test now requires every script `hooks.json` names to be mode 100755 in the git index | ✅ merged (v0.10.1) |
| — | Fixes: `distill ingest` capped `topic`, `name` and each tag (one proposal wrote a 300KB candidate), exits non-zero when nothing survives, and no longer lets `--source` forge a git commit trailer; the registry preserves keys it does not know instead of deleting them; and frontmatter escapes all ten characters `str.splitlines()` breaks on, not two | ✅ merged (v0.10.0) |
